...
func combine_and_atlas(value: bool):
	...
	if value == true: 
		...
		if collision_parent_path != NodePath("") and get_node_or_null(collision_parent_path):
			...
			if collision_parent:
				clean_collisions()
				var node_to_scan_for_original_collisions = target_node.get_parent() # Default to parent of the visual mesh
				if not is_instance_valid(node_to_scan_for_original_collisions): # If target_node has no parent (is root of its own little tree)
					node_to_scan_for_original_collisions = target_node # Then scan target_node itself
				
				if is_instance_valid(node_to_scan_for_original_collisions):
					print("DEBUG: [CollisionCopy] Starting scan for original collision bodies under: %s (will be parented to: %s)" % [node_to_scan_for_original_collisions.name, collision_parent.name])
					copy_collisions_recursive(node_to_scan_for_original_collisions, collision_parent)
				else:
					push_warning("Collision copy: Could not determine a valid node to scan for original collision bodies.")
			else:
				push_warning("Skipping collision copy: collision_parent node (from collision_parent_path) not found or invalid.")
...
func copy_collisions_recursive(node_to_scan: Node, p_new_collision_root: Node):
	if not is_instance_valid(p_new_collision_root):
		push_warning("Collision copy: Invalid new_collision_root. Skipping."); return
	if not p_new_collision_root.is_inside_tree(): 
		push_warning("Collision copy WARNING: new_collision_root node not inside tree. Skipping."); return
	if not is_instance_valid(node_to_scan):
		print("DEBUG: [CollisionCopy] node_to_scan is invalid. Stopping recursion here.")
		return

	# Process current node_to_scan if it's a StaticBody3D
	if node_to_scan is StaticBody3D: 
		var original_static_body: StaticBody3D = node_to_scan
		print("DEBUG: [CollisionCopy] Found StaticBody3D to copy: %s" % original_static_body.name)
		
		var new_static_body := StaticBody3D.new()
		new_static_body.name = original_static_body.name + "_coll_copy" 
		new_static_body.collision_layer = original_static_body.collision_layer
		new_static_body.collision_mask = original_static_body.collision_mask
		# Set global transform to match the original StaticBody3D's position in the world
		new_static_body.global_transform = original_static_body.global_transform 
		
		p_new_collision_root.add_child(new_static_body)
		if Engine.is_editor_hint():
			if get_tree() and get_tree().edited_scene_root:
				new_static_body.owner = get_tree().edited_scene_root
			else:
				push_warning("Collision copy Warning: Could not set owner for new StaticBody3D (edited_scene_root is null).")

		for child_shape_node in original_static_body.get_children():
			if child_shape_node is CollisionShape3D:
				var original_col_shape: CollisionShape3D = child_shape_node
				...
				print("DEBUG: [CollisionCopy]   Copying CollisionShape3D: %s to new body: %s" % [original_col_shape.name, new_static_body.name])
				var new_col_shape := CollisionShape3D.new()
				new_col_shape.name = original_col_shape.name 
				new_col_shape.shape = original_col_shape.shape.duplicate(true)
				# CollisionShapes are positioned relative to their parent CollisionObject3D
				new_col_shape.transform = original_col_shape.transform 
				...
				if Engine.is_editor_hint():
					if get_tree() and get_tree().edited_scene_root:
						...
						push_warning("Collision copy Warning: Could not set owner for new CollisionShape3D (edited_scene_root is null).")
	
	# Recurse for children of the current node_to_scan
	for child_node in node_to_scan.get_children():
		# Avoid recursing into self (if script is on target) or the collision parent itself,
		# though this check might be redundant if node_to_scan is already a specific part of the original model.
		if child_node == self or child_node == p_new_collision_root: continue 
		copy_collisions_recursive(child_node, p_new_collision_root)


func clean_collisions():
	if not collision_parent or not is_instance_valid(collision_parent): 
		print("DEBUG: [CleanCollisions] No collision_parent set or invalid, skipping.")
		return
		
	if collision_parent.get_child_count() > 0:
		print("Cleaning collisions from under: ", collision_parent.name)
		for i in range(collision_parent.get_child_count() - 1, -1, -1): 
			var child = collision_parent.get_child(i)
			if is_instance_valid(child) and child is StaticBody3D and child.name.ends_with("_coll_copy"): 
				print("Removing copied collision body: ", child.name)
				collision_parent.remove_child(child)
				child.queue_free()
	else:
		print("DEBUG: [CleanCollisions] collision_parent has no children to clean.")
